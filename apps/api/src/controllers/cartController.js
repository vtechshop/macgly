...
async function getCart(req, res, next) {
  try {
    ...
    if (cart) await cart.populate('items.product', 'title images slug stock price gstRate');
...
async function addItem(req, res, next) {
  try {
    ...
    let cart = await resolveCart(req);
    if (!cart) {
      cart = new Cart(cartFilter(req));
    }

    const existing = cart.items.find((i) => i.product.toString() === productId);
    if (existing) {
      const newQty = existing.quantity + quantity;
      if (product.stock < newQty) throw new AppError('Insufficient stock', 400, 'OUT_OF_STOCK');
    ...
    await cart.save();
    await cart.populate('items.product', 'title images slug stock price gstRate');
    res.json({ cart });
  } catch (err) { next(err); }
}
