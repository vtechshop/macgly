...
async function cancelOrder(req, res, next) {
  try {
    // MERGE-01: claim the cancellation atomically.
    //
    // Checkout.jsx fires this route from BOTH the Razorpay payment.failed
    // handler and the modal ondismiss handler, and the payment.failed webhook
    // cancels the same order server-side. Three callers can therefore arrive for
    // one order. The previous findOne/check/save let every one of them reach
    // releaseStock, so a failed payment restored the same items two or three
    // times and inflated inventory. The status filter makes exactly one win.
    const order = await Order.findOneAndUpdate(
      ...
      },
      {
        status: 'cancelled',
        cancellation: {
          // Upstream sends 'Payment failed' / 'Payment dismissed by user'.
          reason: req.body?.reason || 'Cancelled by customer',
          cancelledAt: new Date(),
          cancelledBy: req.user._id,
        },
      },
      { new: false },   // pre-image: applyEarnings needs the status it moves from
    );
    ...
    if (!order) {
      ...
      if (existing.status === 'cancelled') {
        // Someone else already cancelled it. That is the expected outcome when
        // the dismiss and failure handlers both fire — not an error, and
        // emphatically not a reason to restore stock again.
        const current = await Order.findById(req.params.id);
        return res.json({ order: current, refundPending: current?.paymentStatus === 'pending_refund' });
      }
      throw new AppError('Order cannot be cancelled at this stage. Contact support if already shipped.', 400, 'INVALID_STATUS');
    }

    // Reverse the money — applyEarnings compares the previous status to the new
    // one, which is why the pre-image is used. This is the only cancellation
    // path that was not already routed through it, so commission rows created at
    // payment were left payable. Earnings are untouched here: this route only
    // allows pre-delivery statuses, so nothing was ever credited.
    ...
    // `order` is the pre-image, so it still reads as pending. Bring the in-memory
    // copy in line with what was just written, so the response body and the
    // notifications below do not report the order as still open. Not saved —
    // the claim already persisted these fields.
