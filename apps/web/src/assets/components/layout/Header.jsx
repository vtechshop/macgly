        ...
        <div className="px-4 sm:px-6 lg:px-10">
          <div className="flex items-center gap-5 h-[88px]">
            ...
            <div className="flex items-center gap-1 ml-auto md:ml-0 shrink-0">
              ...
              {/* WhatsApp */}
              <a
                href="https://wa.me/919944556683?text=Hi!%20I%20have%20a%20question%20about%20your%20products."
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center p-2 rounded-lg hover:bg-secondary-100 transition-colors"
          ...
          <div className="px-4 py-4 space-y-4">
            ...
            {user ? (
              <div className="pt-2 border-t border-secondary-200 space-y-1">
                <p className="text-xs text-secondary-400 font-medium px-1 pb-1">My Account</p>
                <Link to="/dashboard/customer" onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-2 text-sm text-secondary-700 hover:text-primary-600 py-2 px-1 transition-colors">
                  <User size={15} /> Dashboard
                </Link>
                <Link to="/dashboard/customer/orders" onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-2 text-sm text-secondary-700 hover:text-primary-600 py-2 px-1 transition-colors">
                  📦 My Orders
                </Link>
                <Link to="/dashboard/customer/wishlist" onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-2 text-sm text-secondary-700 hover:text-primary-600 py-2 px-1 transition-colors">
                  <Heart size={15} /> Wishlist
                </Link>
                {(user.role === 'vendor' || user.role === 'admin') && (
                  <Link to={user.role === 'admin' ? '/dashboard/admin' : '/dashboard/vendor'} onClick={() => setMenuOpen(false)}
                    className="flex items-center gap-2 text-sm text-primary-600 font-semibold py-2 px-1 transition-colors">
                    🏪 {user.role === 'admin' ? 'Admin Panel' : 'Vendor Dashboard'}
                  </Link>
                )}
              </div>
            ) : (
