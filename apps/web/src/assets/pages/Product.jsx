...
export default function Product() {
  ...
  function askOnWhatsApp() {
    const msg = `Hi, I'm interested in *${product?.title}* — can you share more details, pricing, and availability?\n${window.location.href}`;
    window.open(`https://wa.me/919944556683?text=${encodeURIComponent(msg)}`, '_blank', 'noopener');
  }
          ...
          <div className="bg-primary-50 border border-primary-100 rounded-xl p-4 space-y-1">
            ...
            <div className="pt-1.5 border-t border-primary-100">
              <span className="inline-flex items-center gap-1.5 text-xs text-green-700 bg-green-50 border border-green-200 px-2.5 py-1 rounded-full font-semibold">
                <CheckCircle size={10} /> GST Invoice Available
              </span>
            </div>
          </div>
          ...
          {activeStock > 0 && (
            <div className="space-y-3" ref={buyActionsRef}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs text-secondary-400 shrink-0">Pay via:</span>
                {['UPI', 'Visa', 'Mastercard', 'RuPay', 'Net Banking'].map((m) => (
                  <span key={m} className="text-xs font-semibold px-2 py-0.5 rounded border border-secondary-200 text-secondary-600 bg-white">{m}</span>
                ))}
              </div>

              {/* Buyer protection */}
              <div className="flex items-center gap-2 flex-wrap text-xs text-secondary-500">
                <span className="flex items-center gap-1"><Shield size={12} className="text-green-500 shrink-0" /> Genuine Products</span>
                <span className="text-secondary-200">·</span>
                <span className="flex items-center gap-1"><CheckCircle size={12} className="text-green-500 shrink-0" /> Easy Returns</span>
                <span className="text-secondary-200">·</span>
                <span className="flex items-center gap-1"><Truck size={12} className="text-green-500 shrink-0" /> Pan India Delivery</span>
              </div>

              {/* WhatsApp seller inquiry */}
              <button
                onClick={askOnWhatsApp}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-sm text-white transition-colors shadow"
      ...
      {relatedData?.products?.filter(p => p._id !== product._id).length > 0 && (
        <section className="mt-10 pt-8 border-t border-secondary-100">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link to={`/category/${categorySlug}`} className="text-sm font-semibold text-primary-600 hover:text-primary-700 flex items-center gap-1">
